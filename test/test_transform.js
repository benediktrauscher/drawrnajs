...
var t = require("../src/transform");
var jsdom = require("node-jsdom");   

var seq = "GAGUACAAAUAGUACCG";
var dotbr = "..((((.....))))..";

describe("transformDotBracket", function(){
    it("should return the correct graph", function(done){
        var result = t.transformDotBracket(seq, dotbr);

        assert.equal(result.nodes.length, 17);
        assert.equal(result.links.length, 20);
        var phos = [];
        var hbond = [];
